...
> Python's built-in `sorted()` and `.sort()` use **Timsort** — O(n log n) worst case, O(n) best case.
...
arr.sort(key=lambda x: (x[1], x[0]))   # sort by multiple keys
...
| Scenario | Algorithm |
|---|---|
| General purpose | Python built-in (Timsort) |
| Stable, guaranteed O(n log n) | Merge Sort |
| Average fastest in practice | Quick Sort |
